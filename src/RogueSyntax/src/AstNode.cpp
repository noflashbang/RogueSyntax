...
FunctionLiteral::FunctionLiteral(const RSToken& token, const std::vector<IExpression*>& parameters, const IStatement* body) : IExpression(token), Parameters(parameters), Body(body), Name("Lambda")
{
	SetUniqueId(this);
}
